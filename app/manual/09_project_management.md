...
You can add additional CMake logic at the application level by providing an `app_extra.cmake` file in the root of the application directory.

If `app_extra.cmake` exists it will be included into the project template when the solution is regenerated.  Within `app_extra.cmake` `${PROJECT_NAME}` can be used as in any standard CMake project and `${NAP_ROOT}` points to the root of the NAP installation. 
...
Below is an example of a simple `app_extra.cmake` with an added include path:
```
